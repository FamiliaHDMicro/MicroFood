...
async function verLoja(request, env) {
  try {
    ...
    return new Response(JSON.stringify({ 
      ...
      loja, 
      fotos: fotos.results || [],
      videos_cliente: videosCliente.results || [],
      musicas_cliente: musicasCliente.results || [],
      videos_autorais: videosAutorais,
      musicas_autorais: musicasAutorais
...
async function uploadVideo(request, env) {
  try {
    ...
    await env.DB.prepare(`
      INSERT INTO videos_cliente (id, loja_id, url, r2_key, titulo, data_upload, data_expira, ativa)
      VALUES (?, ?, ?, ?, ?, ?, ?, 1)
    `).bind(idVideo, lojaId, urlPublica, r2Key, titulo, new Date().toISOString().split('T')[0], rotacao > 0 ? dataExpira.toISOString().split('T')[0] : null).run();
...
async function uploadMusica(request, env) {
  try {
    ...
    await env.DB.prepare(`
      INSERT INTO musicas_cliente (id, loja_id, url, r2_key, titulo, artista, data_upload, data_expira, ativa)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, 1)
    `).bind(idMusica, lojaId, urlPublica, r2Key, titulo, artista, new Date().toISOString().split('T')[0], rotacao > 0 ? dataExpira.toISOString().split('T')[0] : null).run();
    ...
    for (const midia of [...fotosExpiradas.results, ...videosExpirados.results, ...musicasExpiradas.results]) {
      if (midia.r2_key) {
        try {
          ...
          console.log('Deletada do R2: ' + midia.r2_key);
        } catch (erro) {
          console.error('Erro ao deletar do R2: ' + midia.r2_key, erro);
        }
      }
      
      const tabela = midia.tipo ? 'midias' : (midia.titulo && midia.artista !== undefined ? 'musicas_cliente' : 'videos_cliente');
      const campoId = midia.tipo ? 'id' : 'id';
      await env.DB.prepare(`UPDATE ${tabela} SET ativa = 0 WHERE ${campoId} = ?`).bind(midia.id).run();
